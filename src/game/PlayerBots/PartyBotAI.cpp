...
bool PartyBotAI::OnSessionLoaded(PlayerBotEntry* entry, WorldSession* sess)
{
    if (!m_race && !m_class)
    {
        WorldPacket data(CMSG_PLAYER_LOGIN);
        data << entry->playerGUID;
        sess->HandlePlayerLoginOpcode(data);
        return true;
    }
...
Player* PartyBotAI::GetPartyLeader() const
{
    ...
    if (Player* originalLeader = ObjectAccessor::GetPlayer(m_leaderGuid))
    {
        if (me->InBattleGround() == originalLeader->InBattleGround())
        {
            ...
            uint64 currentLeaderGuid = pGroup->GetLeaderGUID();
            if (currentLeaderGuid == me->GetGUID() && !me->InBattleGround())
                return nullptr;
        }
...
bool PartyBotAI::RunAwayFromTarget(Unit* pTarget)
{
    ...
    float x, y, z;
    pTarget->GetNearPoint(x, y, z, 0, 25, pTarget->GetAngle(me));
    ...
    if (me->GetDistanceSq(x, y, z) == 0.0f)
        return false;
...
bool PartyBotAI::DrinkAndEat()
{
    ...
    if (!isEating && needToEat)
    {
        ...
        if (SpellEntry const* pSpellEntry = sSpellStore.LookupEntry(PB_SPELL_FOOD))
        {
    ...
    if (!isDrinking && needToDrink)
    {
        ...
        if (SpellEntry const* pSpellEntry = sSpellStore.LookupEntry(PB_SPELL_DRINK))
        {
            me->CastSpell(me, pSpellEntry, true);
            me->RemoveSpellCooldown(pSpellEntry->Id);
        }
...
{
    if (sSpellMgr.IsAreaOfEffectSpell(pSpellEntry) && !m_marksToCC.empty())
        return false;
...
void PartyBotAI::AttackStart(Unit* pVictim)
{
    ...
    if (me->Attack(pVictim, true))
    {
        ...
        me->GetMotionMaster()->MoveChase(pVictim, chaseDist, m_role == ROLE_MELEE_DPS ? pVictim->GetAngle(me) : 0.0f);
    }
}
...
bool PartyBotAI::CrowdControlMarkedTargets()
{
    ...
    for (auto mark : m_marksToCC)
    {
        ...
        {
            if (!pTarget->HasUnitState(UNIT_STAT_LOST_CONTROL) &&
...
void PartyBotAI::OnPacketReceived(WorldPacket packet)
{
...
void PartyBotAI::UpdateAI(uint32 const diff)
{
    ...
    if (!m_initialized)
    {
        ...
        if (m_race && m_class) // temporary character
        {
            ...
            {
                if (!LearnPremadeSpecForClass())
                    LearnNormalSpellsForClass();
    ...
    if (m_role == ROLE_RANGE_DPS && pVictim && !me->IsNonMeleeSpellCast(false) && me->IsStopped() && me->GetDistance(pVictim) > 20.0f)
    {
        if (me->HasInArc(M_PI_F, pVictim))
            me->MonsterMove(pVictim->GetPositionX(), pVictim->GetPositionY(), pVictim->GetPositionZ());
        else
...
void PartyBotAI::UpdateOutOfCombatAI_Paladin()
{
    ...
    if (Unit* pVictim = me->GetVictim())
    {
        if (m_spells.paladin.pAvengersShield &&
            CanTryToCastSpell(pVictim, m_spells.paladin.pAvengersShield))
        {
            if (DoCastSpell(pVictim, m_spells.paladin.pAvengersShield) == SPELL_CAST_OK)
                return;
        }
    }
}
...
void PartyBotAI::UpdateInCombatAI_Paladin()
{
    ...
    if (Unit* pFriend = SelectHealTarget(0.0f, 0.30f))
    {
    ...
    else
    {
        ...
        if (Unit* pVictim = me->GetVictim())
        {
            ...
            if (m_spells.paladin.pHammerOfJustice &&
               (pVictim->IsNonMeleeSpellCast(false) ||
...
void PartyBotAI::UpdateOutOfCombatAI_Shaman()
{
    ...
    {
        if (Player* pTarget = GetPartyLeader())
        {
            if (CanTryToCastSpell(pTarget, m_spells.shaman.pEarthShield))
            {
                if (DoCastSpell(pTarget, m_spells.shaman.pEarthShield) == SPELL_CAST_OK)
                    return;
            }
        }
    }
    ...
    if (m_role != ROLE_HEALER)
    {
        if (Unit* pVictim = me->GetVictim())
        {
            ...
            if (m_spells.shaman.pEarthShock &&
                pVictim->IsNonMeleeSpellCast(false, false, true) &&
...
void PartyBotAI::UpdateOutOfCombatAI_Hunter()
{
    ...
    if (Unit* pVictim = me->GetVictim())
    {
        ...
        if (Pet* pPet = me->GetPet())
        {
            if (!pPet->GetVictim())
            {
                pPet->GetCharmInfo()->SetCommandState(COMMAND_ATTACK);
...
void PartyBotAI::UpdateInCombatAI_Hunter()
{
    if (Unit* pVictim = me->GetVictim())
    {
        ...
        if (m_spells.hunter.pBestialWrath)
        {
            if (Pet* pPet = me->GetPet())
            {
                if (CanTryToCastSpell(pPet, m_spells.hunter.pBestialWrath))
                {
...
void PartyBotAI::UpdateInCombatAI_Mage()
{
    if (Unit* pVictim = me->GetVictim())
    {
            ...
            if (m_role != ROLE_MELEE_DPS)
            {
                ...
                if (!me->HasUnitState(UNIT_STAT_CAN_NOT_MOVE))
                {
                    if (m_spells.mage.pFrostNova &&
                       ...
                       !pVictim->HasUnitState(UNIT_STAT_LOST_CONTROL) &&
        ...
        {
            if (m_spells.mage.pDragonsBreath && me->HasInArc(M_PI_F, pVictim) &&
                CanTryToCastSpell(me, m_spells.mage.pDragonsBreath))
            {
                if (DoCastSpell(pVictim, m_spells.mage.pDragonsBreath) == SPELL_CAST_OK)
                    return;
            }

            if (m_spells.mage.pConeofCold && me->HasInArc(M_PI_F, pVictim) &&
...
void PartyBotAI::UpdateInCombatAI_Priest()
{
    ...
    if (m_role == ROLE_HEALER)
    {
        ...
        {
            if (Player* pTarget = SelectGroupHealTarget(sSpellMgr.GetSpellRadius(m_spells.priest.pCircleOfHealing, 0, true), 50.0f, 70.0f))
            {
                if (CanTryToCastSpell(pTarget, m_spells.priest.pCircleOfHealing))
                {
                    if (DoCastSpell(pTarget, m_spells.priest.pCircleOfHealing) == SPELL_CAST_OK)
                        return;
                }
            }
        }
...
void PartyBotAI::UpdateInCombatAI_Warlock()
{
    if (Unit* pVictim = me->GetVictim())
    {
        if (m_spells.warlock.pDeathCoil &&
           (pVictim->CanReachWithMeleeAutoAttack(me) || pVictim->IsNonMeleeSpellCast(false)) &&
...
void PartyBotAI::UpdateInCombatAI_Warrior()
{
    if (Unit* pVictim = me->GetVictim())
    {
        if (pVictim->IsNonMeleeSpellCast(false, false, true))
        {
        ...
        if (m_spells.warrior.pConcussionBlow &&
           (pVictim->IsNonMeleeSpellCast(false) || pVictim->IsMoving() || (me->GetHealthPercent() < 50.0f)) &&
            ...
            IsWearingShield())
        {
            ...
            if (m_spells.warrior.pDevastate &&
                CanTryToCastSpell(pVictim, m_spells.warrior.pDevastate))
            {
...
bool PartyBotAI::ShouldEnterStealth() const
{
    ...
    if (Player* pLeader = GetPartyLeader())
    {
        if (pLeader->IsDead() || pLeader->HasAura(SPELL_AURA_FEIGN_DEATH) ||
...
void PartyBotAI::UpdateInCombatAI_Rogue()
{
    if (Unit* pVictim = me->GetVictim())
    {
        if (me->HasAuraType(SPELL_AURA_MOD_STEALTH))
        {
            ...
            if (IsRangedDamageClass(pVictim->GetClass()))
            {
        ...
        if (pVictim->IsNonMeleeSpellCast(false))
        {
        ...
        if (m_spells.rogue.pMutilate &&
            CanTryToCastSpell(pVictim, m_spells.rogue.pMutilate))
        {
            if (DoCastSpell(pVictim, m_spells.rogue.pMutilate) == SPELL_CAST_OK)
                return;
        }
        ...
        case FORM_MOONKIN:
        {
                ...
                me->GetDistance(pVictim) > 30.0f)
            {
                me->GetMotionMaster()->MoveChase(pVictim, 25.0f);
                    ...
                    (pVictim->GetVictim() == me) &&
                    !me->HasUnitState(UNIT_STAT_ROOT) && me->IsStopped())
            {
